...
for filename in filename_list:
    ...
    for instance in instance_list:
        ...
        # hyp = ' '.join(hyp_split)
        # instruction = 'Insert a sentence between Obs1="%s" and Obs2="%s", while also containing the keyword "%s".' % (obs1, obs2, verb)
        instruction = "The first sentence is \" %s \" and the last sentence is \" %s \" . Insert a middle sentence with similar style, while also containing the keyword \"%s\"." % (obs1, obs2, verb)

        regex = "<expression> <mask_0> %s <mask_1> </expression>" % (verb)
